    ...
    <div v-if="open" class="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/75 px-4 backdrop-blur-sm">
      <div class="glass-panel w-full max-w-lg p-6 sm:p-8" v-motion :initial="{ opacity: 0, scale: 0.96, y: 14 }" :enter="{ opacity: 1, scale: 1, y: 0 }">
        <div class="flex items-center justify-between">
          <div>
            <p class="pill">Join room</p>
            <h3 class="mt-4 font-display text-3xl text-white">Slip into the table</h3>
          </div>
        ...
        <form class="mt-6 space-y-4" @submit.prevent="handleJoin">
          ...
          <button class="action-button w-full bg-cyan-400 text-slate-950 hover:bg-cyan-300" :disabled="!canJoin">Join room</button>
        </form>
      </div>
    </div>
  </transition>
